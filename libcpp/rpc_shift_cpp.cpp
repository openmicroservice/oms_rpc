    ...
    if (EnumPtr::dynamicCast(pPtr->getTypePtr())) {
        ...
        if (!_checkDefault || pPtr->isRequire() || (!pPtr->hasDefault() && !mPtr && !vPtr)) {
            ...
            if (pPtr->getTypePtr()->isSimple() || (bPtr && bPtr->kind() == Builtin::KindString)) {
                s << TAB << "p->value[\"" << pPtr->getId() << "\"] = " + _namespace + "::XmlOutput::writeXml("
...
string Tars2Cpp::readFromXml(const TypeIdPtr &pPtr, bool bIsRequire) const {
    ...
    if (EnumPtr::dynamicCast(pPtr->getTypePtr())) {
        ...
        s << TAB << "tars::XmlInput::readXml(" << pPtr->getId() << "Len" << ", pObj->value[\"" << pPtr->getId() << "\"]"
          << getSuffix(pPtr);
    } else if (pPtr->getTypePtr()->isPointer()) {
        // "not support";
    } else {
...
string Tars2Cpp::writeToSql(const TypeIdPtr &pPtr) const {
    ostringstream s;
    if (EnumPtr::dynamicCast(pPtr->getTypePtr())) {
        s << TAB << "_mycols[\"" << pPtr->getId() << "\"] = make_pair(tars::TC_Mysql::DB_INT, tars::TC_Common::tostr("
          << pPtr->getId() << "));" << endl;
    }
    ...
    if (bPtr) {
        switch (bPtr->kind()) {
            ...
            case Builtin::KindLong:
                s << TAB << "_mycols[\"" << pPtr->getId()
                  << "\"] = make_pair(tars::TC_Mysql::DB_INT, tars::TC_Common::tostr(" << pPtr->getId() << "));"
                  << endl;
                break;
            case Builtin::KindFloat:
            case Builtin::KindDouble:
                s << TAB << "_mycols[\"" << pPtr->getId()
                  << "\"] = make_pair(tars::TC_Mysql::DB_STR, tars::TC_Common::tostr(" << pPtr->getId() << "));"
                  << endl;
                break;
            case Builtin::KindString:
                s << TAB << "_mycols[\"" << pPtr->getId()
                  << "\"] = make_pair(tars::TC_Mysql::DB_STR, tars::TC_Common::trim(" << pPtr->getId() << "));" << endl;
                break;
            default:
                break;
        }
        ...
        s << TAB << "_mycols[\"" << pPtr->getId()
          << "\"] = make_pair(tars::TC_Mysql::DB_STR, tars::TC_Json::writeValue(tars::JsonOutput::writeJson("
          << pPtr->getId() << ")));" << endl;
    }
...
string Tars2Cpp::readFromSql(const TypeIdPtr &pPtr, bool bIsRequire) const {
    ...
    if (ePtr) {
        s << TAB << pPtr->getId() << " = (" << ePtr->getSid() << ")TC_Common::strto<tars::Int32>(_mysrd[\""
          << pPtr->getId() << "\"]);" << endl;
    }
    ...
    if (bPtr) {
        switch (bPtr->kind()) {
            case Builtin::KindBool:
                s << TAB << pPtr->getId() << " = TC_Common::strto<tars::Bool>(_mysrd[\"" << pPtr->getId() << "\"]);"
                  << endl;
                break;
            case Builtin::KindByte:
            ...
            case Builtin::KindLong:
                ...
                break;
            case Builtin::KindFloat:
                s << TAB << pPtr->getId() << " = TC_Common::strto<tars::Float>(_mysrd[\"" << pPtr->getId() << "\"]);"
                  << endl;
                break;
            case Builtin::KindDouble:
                s << TAB << pPtr->getId() << " = TC_Common::strto<tars::Double>(_mysrd[\"" << pPtr->getId() << "\"]);"
                  << endl;
                break;
            case Builtin::KindString:
        ...
        s << TAB << "tars::JsonInput::readJson(" << pPtr->getId() << ", tars::TC_Json::getValue(_mysrd[\""
...
string Tars2Cpp::writeToJson(const TypeIdPtr &pPtr) const {
    ...
    if (EnumPtr::dynamicCast(pPtr->getTypePtr())) {
        s << TAB << "p->value[\"" << pPtr->getId()
          << "\"] = " + _namespace + "::JsonOutput::writeJson((" + _namespace + "::Int32)"
          << pPtr->getId() << ");" << endl;
    } else if (pPtr->getTypePtr()->isArray()) {
        /*
...
string Tars2Cpp::readFrom(const TypeIdPtr &pPtr, bool bIsRequire) const {
    ...
    if (EnumPtr::dynamicCast(pPtr->getTypePtr())) {
        ...
        s << TAB << "_is.read(" << pPtr->getId() << ", " << "sizeof(" + pPtr->getId() + ")" << ", " << pPtr->getId()
          << "Len";
    } else if (pPtr->getTypePtr()->isPointer()) {
        s << TAB << pPtr->getId() << " = (" << tostr(pPtr->getTypePtr()) << ")_is.cur();" << endl;
        s << TAB << "_is.read(" << pPtr->getId() << ", _is.left(), " << pPtr->getId() << "Len";
    ...
    if (EnumPtr::dynamicCast(pPtr->getTypePtr())) {
        s << TAB << pPtr->getId() << " = (" << tostr(pPtr->getTypePtr()) << ")eTemp" << TC_Common::tostr(pPtr->getTag())
          << ";" << endl;
    }
...
string Tars2Cpp::display(const TypeIdPtr &pPtr) const {
    ostringstream s;
    if (EnumPtr::dynamicCast(pPtr->getTypePtr())) {
        s << TAB << "_ds.display((" + _namespace + "::Int32)" << pPtr->getId() << ",\"" << pPtr->getId() << "\");"
          << endl;;
    } else if (pPtr->getTypePtr()->isArray() || pPtr->getTypePtr()->isPointer()) {
        s << TAB << "_ds.display(" << pPtr->getId() << ", " << pPtr->getId() << "Len" << ",\"" << pPtr->getId()
          << "\");" << endl;
    } else {
...
/*
    ...
    */
    ...
    for (size_t j = 0; j < member.size(); j++) {
        if (member[j]->getTypePtr()->isArray()) {
            ...
            if (vPtr) {
                ...
                if (bPtr && bPtr->kind() != Builtin::KindString) //非内建类型或者string 类型不能memset
                {
                    s << TAB << "memset(" << member[j]->getId() << ", 0, " << "sizeof(" << member[j]->getId() << "));"
                      << endl;
                }
            }
            continue;
    ...
    for (size_t j = 0; j < member.size(); j++) {
        ...
        s << TAB << tostr(member[j]->getTypePtr()) << " " << member[j]->getId() << toStrSuffix(member[j]) << ";"
          << endl;
    ...
    for (size_t j = 0; j < member.size(); j++) {
        ...
        {
            s << "!memcmp(l." << member[j]->getId() << ",r." << member[j]->getId() << ",l." << member[j]->getId()
              << "Len)";
        } else {
            BuiltinPtr bPtr = BuiltinPtr::dynamicCast(member[j]->getTypePtr());
            MapPtr mPtr = MapPtr::dynamicCast(member[j]->getTypePtr());
            ...
            if (mPtr) {
                {
                    ...
                    if (innerPtr &&
                        (innerPtr->kind() == Builtin::KindFloat || innerPtr->kind() == Builtin::KindDouble)) {
                        mapDouble = true;
                    }
                }
                {
...
string Tars2Cpp::generateDispatchAsync(const OperationPtr &pPtr, const string &cn) const {
    ...
    for (size_t i = 0; i < vParamDecl.size(); i++) {
        if (vParamDecl[i]->isOut()) {
            s << TAB << tostr(vParamDecl[i]->getTypeIdPtr()->getTypePtr()) << " "
              << vParamDecl[i]->getTypeIdPtr()->getId() << generateInitValue(vParamDecl[i]->getTypeIdPtr()) << ";"
              << endl;
    ...
    if (_bTrace) {
        ...
        s << TAB
          << "TARS_TRACE(pSptd->getTraceKey(ServantProxyThreadData::TraceContext::EST_CR), TRACE_ANNOTATION_CR, \"\", ServerConfig::Application + \".\" + ServerConfig::ServerName, \""
...
//        }
    ...
    for (size_t i = 0; i < vParamDecl.size(); i++) {
        ...
        string sEnum2Int = (EnumPtr::dynamicCast(vParamDecl[i]->getTypeIdPtr()->getTypePtr())) ? "(" + _namespace +
                                                                                                 "::Int32)" : "";
        if (!vParamDecl[i]->isOut()) {
            //枚举类型转成int
            ...
            if (EnumPtr::dynamicCast(vParamDecl[i]->getTypeIdPtr()->getTypePtr())) {
                s << TAB << sParamName << " = (" << tostr(vParamDecl[i]->getTypeIdPtr()->getTypePtr())
                  << ") tarsAttr.getByDefault<" + _namespace + "::Int32>(\"" << sParamName << "\", " << sEnum2Int
                  << sParamName << ");" << endl;
            } else {
    ...
    if (_bJsonSupport) {
        ...
        s << TAB << "{" << endl;
        INC_TAB;
        s << TAB << _namespace << "::JsonValueObjPtr _jsonPtr = " << _namespace << "::JsonValueObjPtr::dynamicCast("
    ...
    if (_bXmlSupport && tars::TC_Common::matchPeriod(pPtr->getId(), _vXmlIntf)) {
        ...
        s << TAB
          << "tars::XmlValueObjPtr _xmlPtr = tars::XmlValueObjPtr::dynamicCast(tars::TC_Xml::getValue(_current->getRequestBuffer()));"
          << endl;
        for (size_t i = 0; i < vParamDecl.size(); i++) {
            string sParamName = vParamDecl[i]->getTypeIdPtr()->getId();
            if (!vParamDecl[i]->isOut()) {
                //枚举类型转成int
                s << TAB << "tars::XmlInput::readXml(" << sParamName << ", _xmlPtr->value[\"" << sParamName
                  << "\"], true);" << endl;
            } else {
                s << TAB << "tars::XmlInput::readXml(" << sParamName << ", _xmlPtr->value[\"" << sParamName
                  << "\"], false);" << endl;
    ...
    if (_bTrace) {
        ...
        s << TAB
          << "TARS_TRACE(pSptd->getTraceKey(ServantProxyThreadData::TraceContext::EST_SR), TRACE_ANNOTATION_SR, \"\", ServerConfig::Application + \".\" + ServerConfig::ServerName, \""
          << pPtr->getId() << "\", 0, _trace_param_, \"\");" << endl;
        DEL_TAB;
        s << TAB << "}" << endl;
        s << endl;
        ...
        //if (_bJsonSupport && tars::TC_Common::matchPeriod(pPtr->getId(), _vJsonIntf))
    {
        ...
        for (size_t i = 0; i < vParamDecl.size(); i++) {
            string sParamName = vParamDecl[i]->getTypeIdPtr()->getId();
            if (vParamDecl[i]->isOut()) {
                s << TAB << "_p->value[\"" << sParamName << "\"] = " << _namespace << "::JsonOutput::writeJson("
                  << sParamName << ");" << endl;
            }
        }
    ...
    if (_bXmlSupport && tars::TC_Common::matchPeriod(pPtr->getId(), _vXmlIntf)) {
        ...
        for (size_t i = 0; i < vParamDecl.size(); i++) {
            string sParamName = vParamDecl[i]->getTypeIdPtr()->getId();
            if (vParamDecl[i]->isOut()) {
                s << TAB << "_p->value[\"" << sParamName << "\"] = tars::XmlOutput::writeXml(" << sParamName << ");"
                  << endl;
            }
        }
        ...
        if (pPtr->getReturnPtr()->getTypePtr()) {
            BuiltinPtr retPtr = BuiltinPtr::dynamicCast(pPtr->getReturnPtr()->getTypePtr());
            if (retPtr->kind() >= Builtin::KindBool && retPtr->kind() <= Builtin::KindLong) {
                s << TAB << "_p->value[\"ret\"] = tars::XmlOutput::writeXml(" << pPtr->getReturnPtr()->getId() << ");"
                  << endl;
            }
        }
    ...
    if (_bTrace) {
        ...
        s << TAB << "}" << endl;
        s << TAB
          << "TARS_TRACE(pSptd->getTraceKey(ServantProxyThreadData::TraceContext::EST_SS), TRACE_ANNOTATION_SS, \"\", ServerConfig::Application + \".\" + ServerConfig::ServerName, \""
          << pPtr->getId() << "\", 0, _trace_param_, \"\");" << endl;
        DEL_TAB;
        s << TAB << "}" << endl;
        s << endl;
...
string Tars2Cpp::promiseReadFrom(const TypeIdPtr &pPtr, bool bIsRequire) const {
    ...
    if (EnumPtr::dynamicCast(pPtr->getTypePtr())) {
        s << TAB << "ptr->" << pPtr->getId() << " = (" << tostr(pPtr->getTypePtr()) << ")eTemp"
          << TC_Common::tostr(pPtr->getTag()) << ";" << endl;
    }
    if (pPtr->getTypePtr()->isPointer())
        s << TAB << "_is.mapBufferSkip(" << pPtr->getId() << "Len);" << endl;
    return s.str();
...
string Tars2Cpp::generateHAsync(const OperationPtr &pPtr, const string &cn) const {
    ...
    if (_bTrace) {
        ...
        s << TAB << "}" << endl;
        s << TAB
          << "TARS_TRACE(pSptd->getTraceKey(ServantProxyThreadData::TraceContext::EST_CS), TRACE_ANNOTATION_CS, ServerConfig::Application + \".\" + ServerConfig::ServerName, tars_name(), \""
          << pPtr->getId() << "\", 0, _trace_param_, \"\");" << endl;
        DEL_TAB;
        s << TAB << "}" << endl;
    }

    s << TAB << "tars_invoke_async(tars::TARSNORMAL,\"" << pPtr->getId() << "\", _os, context, _mStatus, callback);"
      << endl;
    DEL_TAB;
    s << TAB << "}" << endl;
    s << TAB << endl;
    ...
    s << TAB << "tars::Future< " << cn << "PrxCallbackPromise::Promise" << sStruct << "Ptr > promise_async_"
      << pPtr->getId() << "(";
    for (size_t i = 0; i < vParamDecl.size(); i++) {
        if (!vParamDecl[i]->isOut()) {
            s << generateParamDecl(vParamDecl[i]) << ",";
    ...
    s << TAB << "tars_invoke_async(tars::TARSNORMAL,\"" << pPtr->getId()
      << "\", _os, context, _mStatus, callback, true);" << endl;
    DEL_TAB;
    s << TAB << "}" << endl;
...
string Tars2Cpp::generateH(const OperationPtr &pPtr, bool bVirtual, const string &interfaceId) const {
    ...
    if (bVirtual) {
        ...
        s << TAB << "shared_ptr<" + _namespace + "::ResponsePacket> rep = tars_invoke(tars::TARSNORMAL,\""
        ...
        if (vParamDecl.size() > 0 || pPtr->getReturnPtr()->getTypePtr()) {
            ...
            if (_bTrace) {
                ...
                DEL_TAB;
                s << TAB << "}" << endl;
                s << TAB << "else if(ServantProxyThreadData::TraceContext::ENP_OVERMAXLEN == _trace_param_flag_)"
                  << endl;
                s << TAB << "{" << endl;
                INC_TAB;
                s << TAB << "_trace_param_ = " << G_TRACE_PARAM_OVER_MAX_LEN << ";" << endl;
                ...
                s << TAB << "}" << endl;
                s << TAB
                  << "TARS_TRACE(pSptd->getTraceKey(ServantProxyThreadData::TraceContext::EST_CR), TRACE_ANNOTATION_CR, ServerConfig::Application + \".\" + ServerConfig::ServerName, tars_name(), \""
                  << pPtr->getId() << "\", 0, _trace_param_, \"\");" << endl;
                DEL_TAB;
                s << TAB << "}" << endl;
                s << endl;
    ...
    if (bVirtual) {
        ...
        if (pPtr->getReturnPtr()->getTypePtr()) {
            string sEnum2Int = (EnumPtr::dynamicCast(pPtr->getReturnPtr()->getTypePtr())) ? "(" + _namespace +
                                                                                            "::Int32)" : "";
            s << TAB << "tarsAttr.put(\"\", " << sEnum2Int << "_ret);" << endl;
            s << TAB << "tarsAttr.put(\"tars_ret\", " << sEnum2Int << "_ret);" << endl;
        }
        ...
        if (_bTrace) {
            ...
            s << TAB << "}" << endl;
            s << TAB
              << "TARS_TRACE(current->getTraceKey(), TRACE_ANNOTATION_SS, \"\", ServerConfig::Application + \".\" + ServerConfig::ServerName, \""
              << pPtr->getId() << "\", 0, _trace_param_, \"\");" << endl;
            DEL_TAB;
            s << TAB << "}" << endl;
            s << endl;
...
string Tars2Cpp::generateHPromiseAsync(const InterfacePtr &pInter, const OperationPtr &pPtr) const {
    ...
    for (size_t i = 0; i < vParamDecl.size(); i++) {
        ParamDeclPtr &pPtr = vParamDecl[i];
        if (pPtr->isOut()) {
            s << TAB << tostr(pPtr->getTypeIdPtr()->getTypePtr()) << " " << pPtr->getTypeIdPtr()->getId() << ";"
              << endl;
        }
    }
    ...
    s << TAB << "};" << endl;
    s << TAB << endl;
    s << TAB << "typedef tars::TC_AutoPtr< " << pInter->getId() << "PrxCallbackPromise::Promise" << sStruct
      << " > Promise" << sStruct << "Ptr;" << endl;
    s << endl;
    s << TAB << pInter->getId() << "PrxCallbackPromise(const tars::Promise< " << pInter->getId()
      << "PrxCallbackPromise::Promise" << sStruct << "Ptr > &promise)" << endl;
    s << TAB << ": _promise_" << sStruct << "(promise)" << endl;
    s << TAB << "{}" << endl;
    s << TAB << endl;
    s << TAB << "virtual void " << "callback_" << pPtr->getId() << "(const " << pInter->getId()
      << "PrxCallbackPromise::Promise" << sStruct << "Ptr &ptr)" << endl;
    s << TAB << "{" << endl;
    INC_TAB;
    ...
    INC_TAB;
    s << TAB << "tars::Promise< " << pInter->getId() << "PrxCallbackPromise::Promise" << sStruct << "Ptr > _promise_"
      << sStruct << ";" << endl;
    return s.str();
}
...
string Tars2Cpp::generateDispatchPromiseAsync(const OperationPtr &pPtr, const string &cn) const {
    ...
    s << endl;
    string sStruct = pPtr->getId();
    s << TAB << cn << "PrxCallbackPromise::Promise" << sStruct << "Ptr ptr = new " << cn
...
string Tars2Cpp::generateH(const InterfacePtr &pPtr, const NamespacePtr &nPtr) const {
    ...
    s << TAB << "typedef tars::TC_AutoPtr<" << pPtr->getId() << "PrxCallback> " << pPtr->getId() << "PrxCallbackPtr;"
      << endl;
    s << endl;
    //生成promise异步回调Proxy
    ...
    DEL_TAB;
    s << TAB << "};" << endl;
    s << TAB << "typedef tars::TC_AutoPtr<" << pPtr->getId() << "PrxCallbackPromise> " << pPtr->getId()
    ...
    s << TAB << "typedef tars::TC_AutoPtr<" << pPtr->getId() << "CoroPrxCallback> " << pPtr->getId()
